...
function RouteDecoratorFactory(method: RequestMethod) {
  return (path: string, middlewares: Function[] = []): MethodDecorator =>
    (target: Object, propertyKey: string, descriptor: TypedPropertyDescriptor<any>) => {
      const methodMetadata: MethodRouteMetadata | undefined = Reflect.getMetadata(METHOD_ROUTE_METADATA_STORAGE, target.constructor);
